...
def index():
    ...
    if tradeid is None:
        ...
        if trader_username is None:
            return {"editable": True, "user_id": auth.user_id, "trader_username": "", "trader_id": "",
            ...
            if db(db.auth_user.username == trader_username).count() == 1:
                ...
                return {"editable": True, "user_id": auth.user_id, "trader_id": trader_id,
    ...
    if (trade is None) | ((auth.user_id != trade.sender) & (auth.user_id != trade.receiver)):
    ...
    return {"user_id": auth.user_id, "trader_id": trader_id, "trader_username": trader_username, "editable": editable,
...
def newTrade():
    if (request.vars['receivedItems[]'] is None) | (request.vars['sentItems[]'] is None) | (
        ...
        response.flash = T('Error: incomplete trade, please enter trade items or select other user')

    receiverId = int(request.vars.receivedId)
    senderId = int(request.vars.senderId)
    if int(auth.user_id) != senderId:
        response.flash = T('Error: not logged in for this trade')
    ...
    for string in senderobjects:
        if string != senderId:
            response.flash = T('Error: invalid item ID, please refresh page')
    ...
    session.flash = T('Successfully created trade with ' + receiver_username if receiver_username is not None else 'unknown')
